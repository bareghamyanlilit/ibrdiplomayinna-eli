// Iradardzyunneri ej — news ev announcements mekabanvum en mi hayecakim
// Sort arenq createdAt-ov nakhagnman karigov
// isLatest-y granayin stalinere (arajiny) gratvayin arandznahatak unenum e
"use client";
...
export function Events() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_URL}/news`).then((r) => r.json()).catch(() => []),
      fetch(`${API_URL}/announcements`).then((r) => r.json()).catch(() => []),
    ]).then(([news, ann]) => {
      const all = [...(Array.isArray(news) ? news : []), ...(Array.isArray(ann) ? ann : [])];
  ...
  const getTitle = (item: Item) => item.title[lang] || item.title.am || "";
  const getContent = (item: Item) => item.content[lang] || item.content.am || "";
  ...
  return (
    <div className="container mx-auto grid gap-4 my-8 px-4 py-10">
      {items.map((item, index) => {
        const isLatest = index === 0;
        return (
          <div key={item._id} className="flex flex-col gap-4">
            <H2 className={`text-xl font-bold m-0 mb-1 ${isLatest ? "text-[#004471]" : "text-gray-800"}`} txt={getTitle(item)} />
            <div className={`sm:border-l-4 p-4 rounded-lg shadow-md flex flex-col lg:flex-row gap-4 ${isLatest ? "border-[#004471] bg-blue-50" : "border-gray-300 bg-white"}`}>
              <div className="flex-1 grid gap-3">
                ...
                <div className="text-sm text-gray-500 mt-2">
                  ...
                  {item.upToDate && <span className="ml-2">· Ակտուալ է մինչև՝ {item.upToDate}</span>}
                </div>
              </div>
              {item.video && (
      ...
      {items.length === 0 && <p className="text-center text-gray-400 py-12">Իրադարձություններ չկան</p>}
    </div>
  );
}
