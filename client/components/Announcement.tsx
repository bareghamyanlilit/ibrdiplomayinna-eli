// Haytararutyunneri ej — fetch API-ic, render anenq lezvakov
// formatDate-y arn localization anenq
// upToDate-y vercnum e verjanapahvac amsakatutyuny
"use client";
...
export function Announcement() {
  ...
  const getTitle = (item: AnnouncementItem) => item.title[lang] || item.title.am || "";
  const getContent = (item: AnnouncementItem) => item.content[lang] || item.content.am || "";
  ...
  return (
    <div className="container mx-auto my-8 px-4 py-10">
      {items.length > 0 ? (
        ...
        <P txt={data.events.announcement.havent} className="my-4 mx-2 text-center text-gray-500" />
      )}
    </div>
  );
}
